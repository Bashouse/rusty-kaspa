[![DeepWiki](https://img.shields.io/badge/DeepWiki-bascoinnet%2Frusty--bascoin-blue.svg?logo=data:image/png;base64,)](https://github.com/bascoinnet/rusty-bascoin)
<!-- DeepWiki badge generated by https://deepwiki.ryoppippi.com/ -->
<h1>Bascoin On Rust</h1>

Welcome to the Rust-based implementation of the Bascoin full-node and its ancillary libraries. The contained node release serves as a drop-in replacement to the established <a href="https://github.com/bascoinnet/bascoind">Golang node</a> and to date is the recommended node software for the Bascoin network, introducing developers to the possibilities of Rust in the Bascoin network's context.
...
Your feedback, contributions, and issue reports will be integral to evolving this codebase and continuing its maturity as a reliable node in the Bascoin network.
...
`bascoin-cli` crate provides a cli-driven RPC interface to the node and a
terminal interface to the Rusty Bascoin Wallet runtime. These wallets are
compatible with WASM SDK Wallet API and Bascoin NG projects.
...
## Running the node
  ...
  **Start a mainnet node**
  ...
  ```bash
  cargo run --release --bin bascoind
  # or with UTXO-index enabled (needed when using wallets)
  cargo run --release --bin bascoind -- --utxoindex
  ```
  **Start a testnet node**

  ```bash
cargo run --release --bin bascoind -- --testnet
  ```
...
Using a configuration file
  ...
  ```bash
cargo run --release --bin bascoind -- --configfile /path/to/configfile.toml
# or
cargo run --release --bin bascoind -- -C /path/to/configfile.toml
  ```
...
addpeer = ["10.0.0.1", "1.2.3.4"]
 ...
 Pass the `--help` flag to view all possible arguments
  ...
  ```bash
cargo run --release --bin bascoind -- --help
  ```
</details>
...
wRPC
  ...
  wRPC subsystem is disabled by default in `bascoind` and can be enabled via:
  ...
  **Sidenote:**
  ...
  Rusty Bascoin integrates an optional wRPC
  ...
  Bascoin data structures and is data-structure-version agnostic. You can connect to the
...
<summary>Simulation framework (Simba)</summary>
...
Heap-profiling in `bascoind` and `simba` can be done by enabling `heap` feature and profile using the `--features` argument

```bash
cargo run --bin bascoind --profile heap --features=heap
```
...
Logging in `bascoind` and `simba` can be [filtered](https://docs.rs/env_logger/0.10.0/env_logger/#filtering-results) by either:

1. Defining the environment variable `RUST_LOG`
2. Adding the --loglevel argument like in the following example:

    ```
    (cargo run --bin bascoind -- --loglevel info,bascoin_rpc_core=trace,bascoin_grpc_core=trace,consensus=trace,bascoin_core=trace) 2>&1 | tee ~/rusty-bascoin.log
    ```
